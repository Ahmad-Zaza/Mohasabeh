...
class AdminCustomersController extends CBController
{
    ...
    public function saveRenewalSubscription()
    {
        ...
        $result = $stmt->execute([$data["package_id"], $data["users_count"], $data["warehouses"], $data["currency"], ($data["storage_attached_size"] * 1024), $data["subscription-start-date"], $data["subscription-end-date"]]);
        //---------------------------------------------------------------------//
        //2- clear cache
        try {
            try {
