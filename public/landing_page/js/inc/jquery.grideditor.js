    ...
    $.fn.gridEditor = function (options) {
        ...
        self.each(function (baseIndex, baseElem) {
            ...
            function setup() {
                ...
                mainControls = $('<div class="ge-mainControls" />').insertBefore(htmlTextArea).wrap("<div class='container'></div>");
                ...
                $.each(settings.new_row_layouts, function (j, layout) {
                    var btn = $('<a class="btn btn-xs btn-primary" />')
                        ...
                        .on('click', function () {
                            ...
                            row = row.wrap("<section class='ge-sect'>").wrap("<div class='container bs-container'></div>");
                            init();
                        })
                ...
                $.each(settings.widgets_section, function (j, layout) {
                    ...
                    var widEx = $("<a/>").attr("href", "javascript:void(0);").attr("class", "btn btn-default ge-wid-sect-item-drag grab").addClass(settings.widgets_section[j]).html($ico);
                    widEx.draggable({
                        revert: true,
                        helper: 'clone',
                        start: function () {
                            $(".ui-draggable-dragging").removeClass("grab").addClass("grabbing");
                        }
                    });
                ...
                var btnGroup = $('<div class="btn-group pull-right"/>')
                    .appendTo(wrapper);
                var htmlButton = $('<button title="Edit Source Code" type="button" class="btn btn-xs btn-default gm-edit-mode"><span class="glyphicon glyphicon-chevron-left"></span><span class="glyphicon glyphicon-chevron-right"></span></button>')
                    .on('click', function () {
                    ...
                    .appendTo(btnGroup);
                var previewButton = $('<button title="Preview" type="button" class="btn btn-xs btn-default gm-preview"><span class="glyphicon glyphicon-eye-open"></span></button>')
                    ...
                    .on('click', function () {
                        ...
                        if (!previewButton.hasClass('active')) {
                            ...
                            $(".ge-addRowGroup").css({ "visibility": "visible" });
                            ...
                            $(".ge-addRowGroup").css({ "visibility": "hidden" });
                ...
                if (withoutSave === true) {
                    ...
                    if (dataBg !== "" && dataBg !== undefined) {
                        ...
                        if (dataBgArray[1] !== "" && dataBgArray[1] !== undefined) {
                            ...
                            $image = $val;
                            $imageBox = $("<div/>").addClass("image-box-row").append($("<a/>").attr("href", $image).addClass("html5lightbox").attr("data-thumbnail", $image).append($("<img/>").attr("src", $image).addClass("img-responsive")));
                    ...
                    if (dataBgArray[1] !== "" && dataBgArray[1] !== undefined) {
                        ...
                        if (dataBgArray[0] === "bg-img") {
                            $.ajax({
                                ...
                                data: { id: $valA[$valA.length - 1] },
                                type: "post",
                                success: function (data) {
                                    if (data !== "") {
                                        obj.children().find(".image-box-row").remove();
                                        $imageBox = $("<div/>").addClass("image-box-row").append($("<a/>").attr("href", data).addClass("html5lightbox").attr("data-thumbnail", data).append($("<img/>").attr("src", data).addClass("img-responsive")));
                            ...
                            if ($val !== "" && $val !== undefined) {
                                obj.children().find(".color-box-row").remove();
                                $colorBox = $("<div/>").addClass("color-box-row").css("background", $val);
            ...
            function openSettings() {
                ...
                function OpenInsertImageForBackground() {
                    $("#modalInsertPhotoForBackgroundRow .modal-body").html(`<iframe width="100%" height="400" src="js/includes/filemanager/dialog.php?type=2&field_id=TempImageSelectedForBackgroundRow'&fldr=" frameborder="0" style="overflow: scroll; overflow-x: hidden; overflow-y: scroll; "></iframe>`);
                ...
                returnValE($('#CustomeMarginBottom'), currentColSelected.attr('data-style'), 'margin-bottom');
                returnValE($('#CustomeMarginLeft'), currentColSelected.attr('data-style'), 'margin-left');
                returnValE($('#CustomeMarginRight'), currentColSelected.attr('data-style'), 'margin-right');
                returnValE($('#CustomePaddingTop'), currentColSelected.attr('data-style'), 'padding-top');
                returnValE($('#CustomePaddingBottom'), currentColSelected.attr('data-style'), 'padding-bottom');
                returnValE($('#CustomePaddingRight'), currentColSelected.attr('data-style'), 'padding-right');
                returnValE($('#CustomePaddingLeft'), currentColSelected.attr('data-style'), 'padding-left');
                ...
                $('#BackgroundRowModal .modal-body #desktop_hidden').bootstrapSwitch("state", hidden_md);
            ...
            $('body').on('dblclick', '[data-fe-type]', function (e) {
                ...
                if ($type !== "" && $type !== undefined) {
                    switch ($type) {
                        ...
                        case "map":
                            {
                                ...
                                if ($formula !== undefined || $formula !== "") {
                                    ...
                                    $widget_id_formula = widgetFormula.match(/##wid_con_start##.*##wid_con_end##/g);
                                    $img_id = $widget_id_formula[0].replace("##wid_con_start##", "").replace("##wid_con_end##", "");
                        ...
                        case "form":
                            {
                                $form_id = currentColSelected.children("[data-form-id]").data("form-id");
            ...
            function saveSettings() {
                ...
                if ($('#ContentFullRow').bootstrapSwitch("state") === true) {
                    ...
                    currentColSelected.parent(".bs-container").removeClass("container").addClass("container-fluid");
                } else {
                    currentColSelected.attr("data-content-mode", "container");
                    currentColSelected.parent(".bs-container").removeClass("container-fluid").addClass("container");
                }
                ...
                var hidden_md = $('#BackgroundRowModal .modal-body #desktop_hidden').bootstrapSwitch("state");
                var hidden_sm = $('#BackgroundRowModal .modal-body #tablet_hidden').bootstrapSwitch("state");
                var hidden_xs = $('#BackgroundRowModal .modal-body #mobile_hidden').bootstrapSwitch("state");
            ...
            function OpenFormInsert(form_id) {
                ...
                $.ajax({
                    url: "/landingPage/getAllForms",
                    type: "get",
                    success: function (data) {
                        $select = $("<select/>").addClass("form-control").attr("id", "form_selected");
                        $option = $("<option/>").val("").text("");
                        $select.append($option);
                        if (data.data.length) {
                            $formLists = data.data;
                            for (var x in data.data) {
                                $option = $("<option/>").val(data.data[x]['id']).text(data.data[x]['name']);
            ...
            function OpenEditImage() {
                $("#modalInsertPhoto .modal-body").html(`<iframe width="100%" height="400" src="js/includes/filemanager/dialog.php?type=2&field_id=TempImageSelected'&fldr=" frameborder="0" style="overflow: scroll; overflow-x: hidden; overflow-y: scroll; "></iframe>`);
            ...
            $("#TempImageSelected").change(function () {
                $val = $(this).val();
                if ($val && $val != "") {
                    currentColSelected.children(".ge-content").html("<img src='" + $val + "' alt='" + $val + "' class='img-responsive'>");
            ...
            function EditTextModal(col) {
                ...
                $removeBtn = $('<button/>').attr("type", "button").attr("class", "btn btn-danger pull-left").attr("id", "widRemoveText").html($removeBtnHtml);
