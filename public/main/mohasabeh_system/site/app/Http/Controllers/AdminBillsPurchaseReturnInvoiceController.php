...
class AdminBillsPurchaseReturnInvoiceController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	use GeneralTrait;
	public function cbInit()
	{
			...
			$this->col[] = ["label"=>"المورد","name"=>"debit","join"=>"accounts,name_ar"];
			$this->col[] = ["label"=>"الحساب","name"=>"credit","join"=>"accounts,name_ar"];
        ...
        # END COLUMNS DO NOT REMOVE THIS LINE
			...
			$this->form[] = ['label'=>'المورد','name'=>'debit','type'=>'select2','validation'=>'','width'=>'col-sm-10','datatable'=>'accounts,name_ar','datatable_format'=>"code,' - ',name_ar",'datatable_where' =>'id in ('.implode(',',$this->getPersons()).')'];
	...
	public function hook_query_index(&$query)
	{
		...
		$query = $query->where("bill_type_id", 3);
	...
	public function hook_before_add(&$postdata)
	{
        ...
        $this->checkBill($postdata,'add',null,3);


        //Your code here
		$max = DB::table('bills')->where("bill_type_id", 3)->where('delete_by',0)->where('rotate_year',NULL)->max('code');
        $prefixCode = DB::table('bill_type')->where('id',3)->select('prefix')->first();
		...
		$account = DB::table("accounts")->where("id", "=", $this->getSystemConfigValue('Purchases_Return_Account'))->first();
		$postdata["credit"] = $account->id;
//		$postdata["debit"] = $customerAccountId->account_id;
	...
	public function hook_after_add($id)
	{
		...
		try {
			...
			if($bill->debit != NULL){  //في حال كانت عملية دفع كاش لمورد ليس له حساب
            ...
            $prefixCode = DB::table('inventory_type_id')->where('id',3)->select('prefix')->first();
	...
	public function hook_before_edit(&$postdata, $id)
    {
		 ...
		 //check bill to prevent empty bill or don't has inventory id
		 $this->checkBill($postdata,'edit',$id,3);

//        $customerAccountId = DB::table('persons')->find($postdata['debit']);
//        $this->editCustomerId = $customerAccountId->account_id;
        //Your code here
        $postdata["staff_id"] = CRUDBooster::myId();
        $account = DB::table("accounts")->where("id", "=", $this->getSystemConfigValue('Purchases_Return_Account'))->first();
        $postdata["credit"] = $account->id;
//        $postdata["debit"] = $customerAccountId->account_id;
	...
	public function hook_after_edit($id)
	{
		...
		try {
            ...
            //$base = DB::table("entry_base")->where("bill_id", $bill->id)->get();
			//DB::table("entry_base")->where("bill_id", $bill->id)->delete();

            //foreach ($base as $item) {
            //    DB::table("entries")->where("entry_base_id", $item->id)->delete();
    ...
    public function getPersons(){
        ...
        if($accountsIds == null)
            return  CRUDBooster::redirect($_SERVER['HTTP_REFERER'],"لا يمكنك الدخول إلى صفحة مردور المشتريات حتى يكون لديك مورد واحد على الاقل","warning");
