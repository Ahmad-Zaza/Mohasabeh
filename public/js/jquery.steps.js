...
(function ($, undefined) {
  ...
  function finishStep(wizard, state) {
    ...
    if (checkSectionValid(wizard, state.currentIndex)) {
      saveCustomer(wizard.prop("id"));
    }
  ...
  function checkEmail(wizard, options, state) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $(".error_toast").find(".toast-body").html(res.msg);
   ...
   **/
  function paginationClick(wizard, options, state, index, email = false) {
    if (state.currentIndex == 0 && !email) {
      return checkEmail(wizard, options, state);
    }
    var oldIndex = state.currentIndex;
    if (
      index >= 0 &&
  ...
  function stepClickHandler(event) {
    ...
    if (position > oldIndex) {
            ...
            if (res.state) {
              if (checkSectionValid(wizard, state.currentIndex)) {
                if (anchor.parent().is(":not(.disabled):not(.current)")) {
                  if ($(".current form").length == 0)
                    goToStep(wizard, options, state, position);
                  else if ($(".current form").valid()) {
                    goToStep(wizard, options, state, position);
                  }
                }
                anchor.parent().addClass("checked");
                anchor.parent().prevAll().addClass("checked");
                anchor.parent().nextAll().removeClass("checked");
              }
            } else {
              $(".error_toast").find(".toast-body").html(res.msg);
              $(".error_toast").toast("show");
      ...
      } else if (checkSectionValid(wizard, state.currentIndex)) {
        if (anchor.parent().is(":not(.disabled):not(.current)")) {
          if ($(".current form").length == 0)
            goToStep(wizard, options, state, position);
          else if ($(".current form").valid()) {
...
function checkSectionValid(wizard, index) {
  let form;
  if (wizard.prop("id") == "wizard") form = wizard.find("#wizard-p-" + index);
  else if (wizard.prop("id") == "wizard1")
    form = wizard.find("#wizard1-p-" + index);
