...
class AdminBillsSalesInvoiceController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
    use GeneralTrait;
    public function cbInit()
    {
        ...
        $this->col[] = ["label" => "الزبون", "name" => "debit", "join" => "accounts,name_ar"];
        //			$this->col[] = ["label"=>"اسم الحساب","name"=>"credit","join"=>"accounts,name_ar"];
        ...
        if ($me->id_cms_privileges == 4) {
            ...
            if (count($persons) == 0) {
                $this->form[] = ['label' => 'الزبون', 'name' => 'debit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id = 0'];
            } else {
                $this->form[] = ['label' => 'الزبون', 'name' => 'debit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $this->getPersons()) . ')'];
            }
        } else {
            $this->form[] = ['label' => 'الزبون', 'name' => 'debit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $this->getPersons()) . ')'];
        }
        ...
        $columns[] = ['label' => 'العدد', 'name' => 'quantity', 'type' => 'number', 'step' => 'any', 'required' => true];
        $columns[] = ['label' => 'سعر الوحدة', 'name' => 'unit_price', 'type' => 'number', 'step' => 'any', 'required' => true];
        $columns[] = ['label' => 'المجموع', 'name' => 'subtotal', 'type' => 'number', 'step' => 'any', 'formula' => "[unit_price] * [quantity]", "readonly" => true, 'required' => true];
        
        $this->form[] = ['label' => 'أضافة مادة', 'name' => 'bill_item', 'type' => 'child','validation' => 'required', 'columns' => $columns, 'table' => 'bill_item', 'foreign_key' => 'bill_id'];
        ...
        $this->form[] = ['label' => 'اجمالي الفاتورة', 'name' => 'amount', 'type' => 'number', 'step' => 'any', "readonly" => true, 'required' => true];
        ...
        //$this->form[] = ['label'=>'Customer Name','name'=>'debit','type'=>'select2','validation'=>'required','width'=>'col-sm-10','datatable'=>'accounts,name_ar','datatable_where'=>'major_classification=0'];
    ...
    .end()
		...
		$('#btn-add-table-adafmad').click(function(){
                ...
                var item_id = 0;
                inv = $('#adafmaditem_id').attr('data-inv');
                item_id = $('#adafmaditem_id').attr('data-id');
                qty = $('#adafmaditem_id').attr('data-qty');
    ...
    public function hook_query_index(&$query)
    {
        ...
        $query = $query->where("bill_type_id", 2);
    ...
    public function hook_before_add(&$postdata)
    {
        ...
        //dd($postdata);
        //check bill to prevent empty bill or don't has inventory id
        $this->checkBill($postdata,'add',null,2);
        

        $me = DB::table('cms_users')->find(CRUDBooster::myId());

        //Your code here
        $max = DB::table('bills')->where("bill_type_id", 2)->where('delete_by',0)->where('rotate_year',NULL)->max('code');
        $prefixCode = DB::table('bill_type')->where('id', 2)->select('prefix')->first();

        $postdata["code"] = ($max) ? $max + 1 : 1;
        $postdata['p_code'] = $prefixCode->prefix . '' . $postdata['code'];

        //check if p_code is uniqe
        $this->checkP_Code($postdata['p_code']);

        $postdata["staff_id"] = CRUDBooster::myId();
        $account = DB::table("accounts")->where("id", "=", $this->getSystemConfigValue('Sales_Account'))->first();
        $postdata["credit"] = $account->id;
        ...
        //$postdata['ex_rate'] = $currency->ex_rate;
        $postdata['equalizer'] = (int)$postdata['ex_rate'] * (int)$postdata['amount'];


        $postdata["bill_type_id"] = 2;

        if ($postdata["is_cash"] == 1) {
            // check the user if he is a salesman and connect with his account in tree
            if ($me->id_cms_privileges == 4) {
        ...
        //dd($postdata);
        if ($postdata["is_cash"] == 0 && $postdata["debit"] == null) { //check if debit not null if is_cash is 0
            return  CRUDBooster::redirect(CRUDBooster::mainpath("add/"),"  (زبون)لا يمكن اجراء فاتورة بدفع أجل و لم يتم تحديد حساب العميل","danger");
        }
    ...
    public function hook_after_add($id)
    {
        ...
        DB::beginTransaction();
        try {
            DB::table('bills_files')->where('file_id', null)->delete();
            ...
            if($bill->debit != NULL){  //في حال كانت عملية دفع كاش لزبون ليس له حساب
            ...
            if ($bill->discount > 0) {
                ...
                DB::table('entries')->insert([
                    'entry_base_id' => $entry_base_id,
                    'debit' => $bill->discount,
                    'account_id' => $this->getSystemConfigValue('Granted_Discount'),
                    'credit' => null,
                    'ex_rate' => $bill->ex_rate,
                    'equalizer' => (int)$bill->ex_rate * (int)$bill->discount,
                    'currency_id' => $bill->currency_id,
            ...
            $prefixCode = DB::table('inventory_type_id')->where('id', 2)->select('prefix')->first();
    ...
    public function hook_before_edit(&$postdata, $id)
    {
        ...
        //check bill to prevent empty bill or don't has inventory id
        $this->checkBill($postdata,'edit',$id,2);
        ...
        //dd($postdata);
        if ($postdata["is_cash"] == 0 && $postdata["debit"] == null) { //check if debit not null if is_cash is 0
            return  CRUDBooster::redirect(CRUDBooster::mainpath("edit/".$id),"  (زبون)لا يمكن اجراء فاتورة بدفع أجل و لم يتم تحديد حساب العميل","danger");
        }
    ...
    public function getDelegateNameById($id)
    {
        $itemId = DB::table('persons')->where('account_id', $id)->first();
        $delegate = DB::table('cms_users')->find($itemId->delegate_id);
        //            $data = Array("res" => $delegate, "inventoryId" => $value[0]->q);
    ...
    public function getInventoryByDelegate($id)
    {
        ...
        if($id == '-1'){
            $delegate = DB::table('inventories')->select('id','name_ar')->where('major_classification',0)->get();
        }else{
            $delegate = DB::table('inventories')->select('id','name_ar')->where('delegate_id','=',$id)->get();
        }
    ...
    public function getInventoryItems($id){
        $sql = "SELECT DISTINCT items.id, items.name_ar , items.code  FROM `items`,`item_tracking` WHERE items.id = item_tracking.item_id and item_tracking.source =$id";
        $items = DB::select($sql);	
        return $items;
    }
    public function checkInventoryItem($inventory,$id,$request_qty){
            ...
            //select form data base here
                ...
                if($income < ($outcome + $request_qty)){
                    return "تنبيه : المستودع لا يحوي الكمية المطلوبة من المادة المختارة";
                }
