...
class EntriesHistoryController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	...
	public function getIndex(Request $request)
	{
		...
		if ($request->has('action') && $request->input('action') != -1 && is_numeric($request->action)) {
			...
			if ($request->action == 1) {
				$query->where('entry_base.action', 'edit');
			}
			else { //$request->action == 2
				$query->where('entry_base.action', 'delete');
			}
	...
	public function export($filter)
	{
		...
		if ($request['action'] && $request['action'] != -1 && is_numeric($request['action'])) {
		...
		Excel::create('export_entries_history_report_' . date('Y-m-d H:i:s', time()), function ($excel) use ($data) {
			...
			$excel->sheet('Result', function ($sheet) use ($data) {
    				...
    				$sheet->prependRow(1, array(
				    	...
				    	"date" => trans('modules.action_date'),
				    	"employee_name" => trans('modules.action_by'),
				    	"action" => trans('modules.action'),
				    ));
	...
	public function print($filter)
	{
		...
		//----------------------------------------------
		$data = $query->get();
		return view("report_print.entries_history", array("data" => $data));
