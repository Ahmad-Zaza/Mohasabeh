...
class InventoryAccountingController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	...
	public function getIndex(\Illuminate\Http\Request $request)
	{
		...
		$query = DB::table("item_tracking as t0")
			...
			DB::raw("(SELECT SUM(t1.quantity) FROM item_tracking as t1
                                ...
                                and t1.date <= '" . ($request->date." 23:59:59") . "') as item_in "),
			...
			->leftjoin("inventories as source", "t0.source", "source.id")->where('t0.date', '<=', $request->date." 23:59:59")
		...
		if (in_array($user->roleId,explode(',',config('setting.DELEGATES_ROLES_IDS_HAS_OWN_INVENTORIES')))) {
			if ($request->input('inventory_id') != -1 && $request->input('inventory_id') != null) {
				...
				$query->whereIn('t0.source', $inventories_ids);
		...
		$items_in = $data_collect->sum('item_in');
		$items_out = $data_collect->sum('item_out');
		$items_total = $items_in - $items_out;
		
		$data = $query->offset(0)->limit(config('setting.PAGINATIOM_LIMITATION'))->get();
		
		
		return view("report.inventoryAccounting", array("data" => $data, 'inventories' => $inventories,
			"inventory_id" => $request->input('inventory_id'), 'date' => $request->date, "items" => $items,
			"item_id" => $request->input('item_id'),
			"items_in" => $items_in, "items_out" => $items_out, "items_total" => $items_total
		));
	...
	public function export($filter)
	{
		...
		$all_total = 0;
		foreach ($data as $arr) {
			$temp = array('pCode' => $arr['pCode'], 'nameAr' => $arr['nameAr'], 'sourceInventory' => $arr['sourceInventory']);
			$temp['item_in'] = $arr['item_in'] != null ? $arr['item_in'] : '0';
			$temp['item_out'] = $arr['item_out'] != null ? $arr['item_out'] : '0';
			$temp['total'] = $arr['item_in'] - $arr['item_out'];

			$all_in += $temp['item_in'];
			$all_out += $temp['item_out'];
			$all_total += $temp['total'];
			array_push($new_data, $temp);
		}
		$result = array('pCode' => '', 'nameAr' => '', 'sourceInventory' => '', 'item_in' => $all_in, 'item_out' => $all_out, 'total' => $all_total);
		...
		Excel::create('export_inventory_accounting_' . date('Y-m-d H:i:s', time()), function ($excel) use ($data) {
			...
			$excel->sheet('Result', function ($sheet) use ($data) {
    				...
    				$sheet->prependRow(1, array(
				    	"pCode" => trans('modules.item_code'),
				    	"nameAr" => trans('modules.item_name'),
	...
	public function loadmore($offset, $limit, $filter)
	{
		$filter = json_decode($filter);
		$filter = collect($filter);
		$request = array();
		...
		if ($data->count() > 0) {
			$data->each(function ($item) {
				$item->item_all = number_format((($item->item_in ? $item->item_in : 0)) - (($item->item_out ? $item->item_out : 0)), 2);
	...
	public function print($filter)
	{
		...
		return view("report_print.inventoryAccounting", array("data" => $data, "items_in" => $items_in, "items_out" => $items_out, "items_total" => $items_total));
