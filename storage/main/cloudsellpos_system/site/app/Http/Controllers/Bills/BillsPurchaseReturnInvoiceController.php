...
class BillsPurchaseReturnInvoiceController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$this->col[] = ["label" => trans('modules.supplier'), "name" => "debit", "join" => "accounts,name_ar"];
		...
		$this->form[] = ['label' => trans('modules.supplier'), 'name' => 'debit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $user->getSuppliersAccountsIds()) . ')'];
		...
		$this->script_js .=" 
			...
			var  _YOU_MUST_CHOOSE_SUPPLIER= '".trans('alert.you_must_choose_supplier_if_bill_isnot_cash')."';
		 ...
		 */
		$this->load_js = array();
		$this->load_js[]= asset("js/modules_js/bills/bills_purchase_return_invoice_script.js");
		if(CRUDBooster::isManager()){
            $this->load_js[] = asset("js/modules_js/bills/bills_purchase_return_invoice_script2.js");
        }
		if (CRUDBooster::getCurrentMethod() == 'getEdit') {
            $this->load_js[] = asset("js/modules_js/bills/bills_purchase_return_invoice_script3.js");
        }
		/*
		 | ----------------------------------------------------------------------
	...
	public function hook_before_add(&$postdata)
	{
		...
		$max = Bill::where("bill_type_id", 3)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
		$prefixCode = BillType::where('id', 3)->select('prefix')->first();
		...
		$account = Account::where("id", "=", $this->getSystemConfigValue('Purchases_Return_Account'))->first();
		...
		if ($postdata["is_cash"] == 0 && $postdata["debit"] == null) { //check if debit not null if is_cash is 0
			...
			return CRUDBooster::redirect(CRUDBooster::mainpath("add/"), trans('messages.no_save_isnot_cash_bill_if_donot_select_supplier'), "danger");
		}
	...
	public function hook_after_add($id)
	{
		...
		try {
			...
			$prefixCode = ItemTrackingType::where('id', 3)->select('prefix')->first();
	...
	public function hook_before_edit(&$postdata, $id)
	{
		...
		if ($postdata["is_cash"] == 0 && $postdata["debit"] == null) { //check if debit not null if is_cash is 0
			$this->setItemsDataToSession();
			return CRUDBooster::redirect(CRUDBooster::mainpath("edit/" . $id), trans('messages.no_save_isnot_cash_bill_if_donot_select_supplier'), "danger");
		}
