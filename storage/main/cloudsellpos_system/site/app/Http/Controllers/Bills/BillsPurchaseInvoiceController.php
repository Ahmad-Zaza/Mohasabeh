...
class BillsPurchaseInvoiceController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
    ...
    public function cbInit()
    {
        ...
        $this->col[] = ["label" => trans('modules.supplier'), "name" => "credit", "join" => "accounts,name_ar"];
        ...
        $this->form[] = ['label' => trans('modules.supplier'), 'name' => 'credit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $user->getSuppliersAccountsIds()) . ')'];
        ...
        $this->form[] = ['label' => trans('modules.bill_type'), 'name' => 'bill_type_id', 'type' => 'text', 'validation' => 'required', 'width' => 'col-sm-10', 'style' => 'display:none', 'value' => '1'];
         ...
         */
        $this->load_js = array();
        $this->load_js[] = asset("js/modules_js/bills/bills_purchase_invoice_script.js");
        if(CRUDBooster::isManager()){
            $this->load_js[] = asset("js/modules_js/bills/bills_purchase_invoice_script2.js");
        }

        if (CRUDBooster::getCurrentMethod() == 'getEdit') {
            $this->load_js[] = asset("js/modules_js/bills/bills_purchase_invoice_script3.js");
        }
    ...
    public function hook_before_add(&$postdata)
    {
        ...
        $max = Bill::where("bill_type_id", 1)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
        $prefixCode = BillType::where('id', 1)->select('prefix')->first();
        ...
        $account = Account::where("id", "=", $this->getSystemConfigValue('Purchases_Account'))->first();
    ...
    public function hook_after_add($id)
    {
        ...
        try {
            ...
            $prefixCode = ItemTrackingType::where('id', 1)->select('prefix')->first();
