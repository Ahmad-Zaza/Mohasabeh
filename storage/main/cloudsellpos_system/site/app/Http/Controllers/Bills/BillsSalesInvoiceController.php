...
class BillsSalesInvoiceController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
    ...
    public function cbInit()
    {
        ...
        $this->col[] = ["label" => trans('modules.customer'), "name" => "debit", "join" => "accounts,name_ar"];
        $this->col[] = ["label" => trans('modules.account'), "name" => "credit", "join" => "accounts,name_ar"];
        ...
        $this->form[] = ['label' => trans('modules.customer'), 'name' => 'debit', 'type' => 'select2', 'validation' => '', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $user->getCustomersIdsInProcess()) . ')'];
        ...
        $columns[] = ['label' => trans('modules.unit_price'), 'name' => 'unit_price', 'type' => 'number', 'step' => 'any', 'required' => true];
        $columns[] = ['label' => trans('modules.subtotal'), 'name' => 'subtotal', 'type' => 'number', 'step' => 'any', 'formula' => "[unit_price] * [quantity]", "readonly" => true, 'required' => true];


        $this->form[] = ['label' => trans('modules.add_item_en_ar'), 'name' => 'bills_items', 'type' => 'child', 'validation' => 'required', 'columns' => $columns, 'table' => 'bills_items', 'foreign_key' => 'bill_id'];
        ...
        $this->form[] = ['label' => trans('modules.bill_amount'), 'name' => 'amount', 'type' => 'number', 'step' => 'any', "readonly" => true, 'required' => true];
        ...
        $this->script_js .=" 
            ...
            var  _AVILABLE_QTY_IN_INVENTORY = '".trans('alert.avilable_qty_in_inventory')."';
            var  _THIS_ITEM_IS_FREE = '".trans('alert.this_item_is_free')."';
         ...
         */
        $this->load_js = array();
        $this->load_js[] = asset("js/modules_js/bills/bills_sales_invoice_script1.js");
        if (CRUDBooster::isManager()) {
            $this->load_js[] = asset("js/modules_js/bills/bills_sales_invoice_script2.js");    
        }
        if (CRUDBooster::getCurrentMethod() == 'getEdit') {
            $this->load_js[] = asset("js/modules_js/bills/bills_sales_invoice_script3.js");    
        }
        /*
         | ----------------------------------------------------------------------
    ...
    public function hook_before_add(&$postdata)
    {
        ...
        $user = CRUDBooster::getUser();
        //Your code here
        $max = Bill::where("bill_type_id", 2)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
        $prefixCode = BillType::where('id', 2)->select('prefix')->first();
        ...
        $account = Account::where("id", "=", $this->getSystemConfigValue('Sales_Account'))->first();
    ...
    public function hook_after_add($id)
    {
            ...
            DB::beginTransaction();
            try {
                BillFile::where('file_id', null)->delete();
                ...
                $prefixCode = ItemTrackingType::where('id', 2)->select('prefix')->first();
