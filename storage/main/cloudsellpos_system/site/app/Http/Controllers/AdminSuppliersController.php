...
use Session;
	...
	class AdminSuppliersController extends \crocodicstudio_voila\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'الحساب الأب','name'=>'account_id','type'=>'select2','validation'=>'required','width'=>'col-sm-10','datatable'=>'accounts,name_ar','datatable_where'=>'major_classification=1'];	

			$this->form[] = ['label'=>'المندوب','name'=>'delegate_id','type'=>'select2','width'=>'col-sm-10','datatable'=>'cms_users,name','datatable_where'=>'id_cms_privileges=4'];
...
";
	    ...
	    */
	    public function hook_before_edit(&$postdata,$id) {
	        //Your code here
			$supplier_account_id = DB::table('persons')->where('id',$id)->first()->account_id;
			//dd($supplier_account_id);
			
			$parent_id = $postdata['account_id'];
			$code = Account::getCode($parent_id); 
			//change parent_id and code for supplier account
			DB::table('accounts')->where('id',$supplier_account_id)->update([
				"parent_id"=>$parent_id,
				"code"=>$code
			]);
	    ...
	    */
	    public function hook_before_delete($id) {
			...
			if($res->count() > 0){
				return  CRUDBooster::redirect($_SERVER['HTTP_REFERER']," هذا المورد مرتبط بقيود . لايمكن حذفه","warning");
			}else{
