...
class ExchangeVoucherController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if($user->hasBox == 'own' || $user->hasBox == 'general' ) { //مندوب
			$this->form[] = ['label' => trans('modules.debit_account'), 'name' => 'debit', 'type' => 'select2', 'width' => 'col-sm-10', 'validation' => 'required', 'datatable' => 'accounts,name_ar', 'datatable_where' => 'major_classification = 0', 'datatable_where' => 'id = ' . $user->boxAccount];
		...
		$this->form[] = ['label' => trans('modules.credit_currency'), 'name' => 'currency_id', 'type' => 'select2', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'currencies,name_ar','datatable_where' =>'active=1'];
		$this->form[] = ['label' => trans('modules.credit_amount'), 'name' => 'amount', 'type' => 'number', 'step' => 'any', 'validation' => 'required|min:1', 'width' => 'col-sm-10', 'value' => 0];
		...
		$this->form[] = ['label' => trans('modules.ex_rate'), 'name' => 'ex_rate', 'type' => 'number', 'step' => 'any', 'validation' => 'required|min:0.001', 'width' => 'col-sm-10', "help" => trans('help.voucher_ex_rate_help'), 'value' => 1];
		 ...
		 */
		$this->load_js = array();
		$this->load_js[] = asset("js/modules_js/vouchers/exchange_voucher_script1.js");
		if (CRUDBooster::isSuperAdmin()) {
			$this->load_js[] = asset("js/modules_js/vouchers/exchange_voucher_script2.js");
		}
		if (CRUDBooster::getCurrentMethod() != 'getEdit') {
			$this->load_js[] = asset("js/modules_js/vouchers/exchange_voucher_script3.js");
		}
	...
	public function hook_query_index(&$query)
	{
		...
		if (in_array($user->roleId,[3,7])) { //sales manager & factory cashier show all vouchers except vouchers build by admins or managers
			$query->whereNotIn('vouchers.staff_id',$this->getAdminsIds());
		}

		$query->where("voucher_type_id", 6);
	...
	{
		//dd($postdata);
		$this->checkVoucher($postdata, 'add', null, 6);

		if($postdata['currency_id'] == $postdata['opposite']){
			return CRUDBooster::redirect(CRUDBooster::mainpath("add/"), trans('messages.exchange_voucher_failed_because_credit_currency_simillar_to_opposite_currency'), "warning");
		}

		$account_has_enough = $this->checkAccountHasAmount($postdata['credit'],$postdata['amount'],$postdata['currency_id'],'add');
		if($account_has_enough == false){
			return CRUDBooster::redirect(CRUDBooster::mainpath("add/"), trans('messages.exchange_voucher_failed_because_account_doesnot_have_enough'), "warning");
		}

		$max = Voucher::where("voucher_type_id", 6)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
		$prefixCode = VoucherType::where('id', 6)->select('prefix')->first();
	...
	public function hook_before_edit(&$postdata, $id)
	{
		...
		$this->checkVoucher($postdata, 'edit', $id, 6);
		if($postdata['currency_id'] == $postdata['opposite']){
			return CRUDBooster::redirect(CRUDBooster::mainpath("edit/$id"), trans('messages.exchange_voucher_failed_because_credit_currency_simillar_to_opposite_currency'), "warning");			
		}

		$account_has_enough = $this->checkAccountHasAmount($postdata['credit'],$postdata['amount'],$postdata['currency_id'],'edit',$id);
		if($account_has_enough == false){
			return CRUDBooster::redirect(CRUDBooster::mainpath("edit/$id"), trans('messages.exchange_voucher_failed_because_account_doesnot_have_enough'), "warning");
		}
