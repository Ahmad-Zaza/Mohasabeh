<?php 
	...
	class TransferVouchersController extends \crocodicstudio_voila\crudbooster\controllers\CBController
    {
        ...
        public function cbInit()
        {
            ...
            */
            $this->load_js = array();
            $this->load_js[] = asset("js/modules_js/vouchers/transfer_voucher_script1.js");
            if(CRUDBooster::getCurrentMethod() != 'getEdit'){
                $this->load_js[] = asset("js/modules_js/vouchers/transfer_voucher_script2.js");
            }
        ...
        */
        public function hook_query_index(&$query)
        {
            ...
            if (in_array($user->roleId,[3,7])) { //sales manager & factory cashier show all vouchers except vouchers build by admins or managers
                ...
                $query->where('vouchers.staff_id', $user->id);   
            }*/

            $query->where('vouchers.credit', $user->boxAccount);
        ...
        */
        ...
        {
            $this->checkVoucher($postdata,'add',null,3);


            $postdata["staff_id"] = CRUDBooster::myId();

            $max = Voucher::where("voucher_type_id", 3)->where('action',NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
            $prefixCode = VoucherType::where('id', 3)->select('prefix')->first();
            ...
            $ex_rate = Currency::where('id',$postdata["currency_id"])->first()->ex_rate;
		    $postdata["ex_rate"] =$ex_rate;
		    $postdata["equalizer"] = $postdata["amount"] * $ex_rate;

            //check credit box 
            $vfunc=new VouchersFunctionsController();
            $res= $vfunc->checkAccountBalance($postdata['currency_id'], $postdata['amount'],$postdata['credit']);
            $result=$res->original;
            if($result['res']){
            ...
            $postdata['status']=0; //voucher status is wait to debit account confirm receipt amount
        ...
        public function hook_after_add($id)
        {
            ...
            try {
                ...
                Entry::insert([
                    ...
                    'status'=>0,//make debit entry inactive when amount not receipt yet.
                    'cycle_id' => Session::get('display_cycle'),
                    'create_by'=> CRUDBooster::myId()
                ...
                Entry::insert([
                    ...
                    'ex_rate' => $voucher->ex_rate,
					'equalizer' => $voucher->equalizer,
                    'status'=>1,//make credit entry active.
                    'cycle_id' => Session::get('display_cycle'),
					'create_by'=> CRUDBooster::myId()
        ...
        */
        public function hook_before_edit(&$postdata, $id)
        {
            ...
            $this->checkVoucher($postdata,'edit',$id,3);
            ...
            //check credit box 
            $vfunc=new VouchersFunctionsController();
            $res= $vfunc->checkAccountBalance($postdata['currency_id'], $postdata['amount'],$postdata['credit'],$id);
            $result=$res->original;
    ...
    | Hook for execute command after delete public static function called
        ...
        */
        ...
        public function getEdit($id)
        {
            ...
            if ($voucher->status != 0) {
				return CRUDBooster::redirect($_SERVER['HTTP_REFERER'], trans('messages.you_cannot_edit_transfer_voucher_because_amount_is_receipted'), "warning");
			}
        ...
        public function getDelete($id)
        {
            ...
            if ($voucher->status != 0) {
				return CRUDBooster::redirect($_SERVER['HTTP_REFERER'], trans('messages.you_cannot_delete_transfer_voucher_because_amount_is_receipted'), "warning");
			}
