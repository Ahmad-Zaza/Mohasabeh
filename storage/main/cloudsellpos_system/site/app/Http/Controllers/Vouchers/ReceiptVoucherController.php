...
class ReceiptVoucherController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
    ...
    public function cbInit()
    {
        ...
        $this->col[] = ["label" => trans('modules.majar_account'), "name" => "debit", "join" => "accounts,name_ar"];
        $this->col[] = ["label" => trans('modules.opposite_account'), "name" => "credit", "join" => "accounts,name_ar"];
        ...
        # START FORM DO NOT REMOVE
        $this->form = [];
        $this->form[] = ['label' => trans('modules.transfer_number'), 'name' => 'voucher_number', 'type' => 'text', 'validation' => '', 'width' => 'col-sm-10'];

        $this->form[] = ['label' => trans('modules.account'), 'name' => 'credit', 'type' => 'select2', 'validation' => 'required', 'width' => 'col-sm-10', 'datatable' => 'accounts,name_ar', 'datatable_format' => "code,' - ',name_ar", 'datatable_where' => 'id in (' . implode(',', $user->getAccountsIdsForVouchers()) . ')'];
        ...
        $this->form[] = ['label' => trans('modules.receved_currency'), 'name' => 'currency_id', 'type' => 'select2', 'validation' => 'required', 'width' => 'col-sm-10', 'datatable' => 'currencies,name_ar','datatable_where' =>'active=1'];
        $this->form[] = ['label' => trans('modules.receved_value'), 'name' => 'amount', 'type' => 'number', 'step' => 'any', 'validation' => 'required|min:1', 'width' => 'col-sm-10', 'value' => 0];
        ...
        $this->form[] = ['label' => trans('modules.ex_rate'), 'name' => 'ex_rate', 'type' => 'number', 'step' => 'any', 'validation' => 'required|min:0.001', 'width' => 'col-sm-10', "help" => trans('help.receipt_voucher_ex_rate_help'), 'value' => 1];
        $this->form[] = ['label' => trans('modules.receved_value_from_account'), 'name' => 'equalizer', 'type' => 'number', 'step' => 'any', 'validation' => 'required|min:0', 'width' => 'col-sm-10', 'value' => 0 ,'readonly'=>true];
         ...
         */
        $this->load_js = array();
        $this->load_js[] = asset("js/modules_js/vouchers/receipt_voucher_script1.js");
        if (CRUDBooster::isSuperAdmin()) {
            $this->load_js[] = asset("js/modules_js/vouchers/receipt_voucher_script2.js");
        }
        if (CRUDBooster::getCurrentMethod() != 'getEdit') {
            $this->load_js[] = asset("js/modules_js/vouchers/receipt_voucher_script3.js");
        }
    ...
    {
        $this->checkVoucher($postdata, 'add', null, 1);

        $user = CRUDBooster::getUser();

        $max = Voucher::where("voucher_type_id", 1)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
        $prefixCode = VoucherType::where('id', 1)->select('prefix')->first();
        ...
        if ($user->hasBox == 'own') {
            $postdata['debit'] = $user->boxAccount;
        }
        else {
    ...
    {
        //dd($postdata);
        $this->checkVoucher($postdata, 'edit', $id, 1);
