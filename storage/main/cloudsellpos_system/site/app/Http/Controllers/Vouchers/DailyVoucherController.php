...
class DailyVoucherController extends \crocodicstudio_voila\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if (CRUDBooster::getUser()->showAccountsStatus == 'all') {
			$this->form[] = ['label' => trans('modules.debit_account'), 'name' => 'debit', 'type' => 'select2', 'width' => 'col-sm-10', 'validation' => 'required', 'datatable' => 'accounts,name_ar', 'datatable_where' => 'major_classification = 0'];
			$this->form[] = ['label' => trans('modules.credit_account'), 'name' => 'credit', 'type' => 'select2', 'width' => 'col-sm-10', 'validation' => 'required', 'datatable' => 'accounts,name_ar', 'datatable_where' => 'major_classification = 0'];
		}
		else { // delegates , sales manager, factory cashier
			$dv_accountsIds = $user->getAccountsIdsForDailyVouchers();
			$this->form[] = ['label' => trans('modules.debit_account'), 'name' => 'debit', 'type' => 'select2', 'width' => 'col-sm-10', 'validation' => 'required', 'datatable' => 'accounts,name_ar', 'datatable_where' => 'major_classification = 0', 'datatable_where' => 'id in (' . implode(',', $dv_accountsIds) . ')'];
			$this->form[] = ['label' => trans('modules.credit_account'), 'name' => 'credit', 'type' => 'select2', 'width' => 'col-sm-10', 'validation' => 'required', 'datatable' => 'accounts,name_ar', 'datatable_where' => 'major_classification = 0', 'datatable_where' => 'id in (' . implode(',', $dv_accountsIds) . ')'];
		}
		 ...
		 */
		$this->load_js = array();
		$this->load_js[] = asset("js/modules_js/vouchers/daily_voucher_script1.js");
		if (CRUDBooster::isSuperAdmin()) {
			$this->load_js[] = asset("js/modules_js/vouchers/daily_voucher_script2.js");
		}
		if (CRUDBooster::getCurrentMethod() != 'getEdit') {
			$this->load_js[] = asset("js/modules_js/vouchers/daily_voucher_script3.js");
		}
	...
	{
		//dd($postdata);
		$this->checkVoucher($postdata, 'add', null, 5);
		$max = Voucher::where("voucher_type_id", 5)->where('action', NULL)->where('cycle_id', Session::get('display_cycle'))->max('code');
		$prefixCode = VoucherType::where('id', 5)->select('prefix')->first();
	...
	public function hook_before_edit(&$postdata, $id)
	{
		...
		$this->checkVoucher($postdata, 'edit', $id, 5);
