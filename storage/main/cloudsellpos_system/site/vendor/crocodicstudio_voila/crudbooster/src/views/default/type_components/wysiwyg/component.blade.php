...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var button = ui.button({
            ...
            click: function() {
            //fesal
                $("#modalInsertPhotoEditor .modal-body").html(`<iframe width="100%" height="400" src="/js/includes/filemanager/dialog.php?type=2&multiple=0&field_id=input_{{$name}}" frameborder="0" style="overflow: scroll; overflow-x: hidden; overflow-y: scroll; "></iframe>`);
    ...
    function selectElement(id) {
        ...
        $("#input_{{$name}}").on("change", function () {
            ...
            if(is_empty)
            {
            $("#textarea_{{$name}}").summernote('pasteHTML', '<img src="'+$(this).val()+'" style="width:100px;height:100px;"> ');                    
            }
...
<!-- <a class="btn btn-primary" data-toggle="modal" onclick="openIfram('{{$name}}','{{CRUDBooster::getCurrentModule()->table_name}}','{{CRUDbooster::getCurrentId()}}')" data-target="#model_scrach">edit from scratch</a> -->
    <textarea id='textarea_{{$name}}'  {{$required}} {{$readonly}} {{$disabled}} name="{{$form['name']}}" class='form-control'
...
<div class="modal fade" id="modelForms" >
    ...
    function openIfram(field_name,table_name,current_id){
        ...
        var url="{{URL::to('/')}}"+"/modules/fromscratch?filed_name="+field_name+"&table_name="+table_name+"&current_id="+current_id;
