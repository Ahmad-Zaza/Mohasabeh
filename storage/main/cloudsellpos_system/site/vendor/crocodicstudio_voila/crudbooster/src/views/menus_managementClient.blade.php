...
@section('content')
            ...
            $(function () {
                ...
                var sortactive = $(".draggable-menu").sortable({
                    ...
                    onDrop: function ($item, container, _super) {
                        ...
                        $.post("{{route('MenusClientControllerPostSaveMenu')}}", {menus: jsonString, isActive: isActive}, function (resp) {
    ...
    <a aria-expanded="false" aria-haspopup="true" role="button" data-toggle="dropdown" class="dropdown-toggle" href="#">
      <span id="selected">
      @php
      $lang_id=(Request::get('lang'))?:1;
         $lang = DB::table('languages')->where('id',$lang_id)->first();
      @endphp                                  
      {{$lang->name}}
      </span><span class="caret"></span></a>
        <ul class="dropdown-menu">
         @foreach($languages as $item)
            <li><a href="{{url(config('crudbooster.ADMIN_PATH').'/menu_management_client?lang='.$item->id)}}">{{$item->name}}</a></li>
         @endforeach
        </ul>
    </div>
                                ...
                                <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                                    <i class='{{($menu->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$menu->icon}}'></i> {{$menu->name}} <span
                                            class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                  href='{{route("MenusClientControllerGetEdit",["id"=>$menu->id])}}?lang={{$lang_id}}&return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{CRUDBooster::deleteConfirm(route("MenusClientControllerGetDelete",["id"=>$menu->id]))}}'
                                                href='javascript:void(0)'></a></span>
                                    <br/><em class="text-muted">
                                        <small><i class="fa fa-users"></i> &nbsp; {{$menu->path}}</small>
                                    </em>
                                </div>
                                <ul>
                                    @if($menu->children)
                                            ...
                                            <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                                                <div class='{{$child->is_dashboard?"is-dashboard":""}}'
                                                    ...
                                                    <span class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                                href='{{route("MenusClientControllerGetEdit",["id"=>$child->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                                title="Delete" class='fa fa-trash'
                                                                onclick='{{CRUDBooster::deleteConfirm(route("MenusClientControllerGetDelete",["id"=>$child->id]))}}'
                                                                href='javascript:void(0)'></a></span>
                                                    <br/><em class="text-muted">
                                                        <small><i class="fa fa-users"></i> &nbsp; {{$child->path}}</small>
                                                    </em>
                                                </div>
                                            </li>
                                        @endforeach
                        ...
                        @foreach($menu_inactive as $menu)
                            <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                                <div><i class='{{$menu->icon}}'></i> {{$menu->name}} <span class='pull-right'><a class='fa fa-pencil' title='Edit'
                                                                                                                 href='{{route("MenusClientControllerGetEdit",["id"=>$menu->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>&nbsp;&nbsp;<a
                                                title='Delete' class='fa fa-trash'
                                                onclick='{{CRUDBooster::deleteConfirm(route("MenusClientControllerGetDelete",["id"=>$menu->id]))}}'
